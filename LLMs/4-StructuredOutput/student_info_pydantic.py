...
class Review(BaseModel):
    key_words: list[str] = Field(description="Write down the keywords on which the review is based.")
    summary: str = Field(description="A brief summary of the review.")
    sentiment: str = Field(description="The sentiment of the review (positive, neutral, negative).")
    pros: str = Field(description="List the pros mentioned in the review.")
    cons: str = Field(description="List the cons mentioned in the review.")
    recommendation_score: float = Field(description="A score from 1 to 10 indicating the likelihood of recommending this product.")
...
for review_text in oneplus_nord_5_reviews:
    ...
    results.append(output.model_dump())  # CRITICAL FIX

with open("reviews_pydantic.csv", "w", newline="", encoding="utf-8") as f:
